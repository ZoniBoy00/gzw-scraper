...
def apply_ballistics_penetration(items: List[Dict[str, Any]], thresholds: Dict[str, Optional[str]]) -> int:
    """Apply Ballistics-table thresholds to scraped ammo items."""
    updated = 0
    for item in items:
        key = _normalise_ammo_name(item.get("name", ""))
...
def run_full_scrape() -> bool:
    ...
    if "ammo.json" in merged:
        ballistics = scrape_ballistics_penetration()
        if ballistics:
            updated = apply_ballistics_penetration(merged["ammo.json"], ballistics)
